...
export async function POST(req: NextRequest){
  ...
  try {
    ...
    const r = await fetch(url, { method:'POST', headers:{ 'content-type':'application/json', 'api-key': process.env.AZURE_OPENAI_API_KEY! }, body: JSON.stringify({ messages:[{role:'system',content:'You are a helpful assistant for a Singapore Government procurement and contract analysis portal.'},{role:'user',content: message||'Hello'}], temperature:0.2, max_tokens:400 }) });
